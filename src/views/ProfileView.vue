<template>
  <div class="h-full bg-main overflow-y-auto custom-scrollbar transition-colors duration-500">
      ...
      <header class="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-line pb-8 lg:pb-10">
        <div class="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-6">
          <div class="relative group">
            ...
            <button v-if="profileForm.avatar" @click="openRemoveAvatarModal" class="absolute -top-2 -right-2 p-1.5 bg-card border border-line text-rose-500 rounded-full shadow-md hover:bg-rose-500 hover:text-white hover:scale-110 transition-all z-10 cursor-pointer" :title="$t('profile.removePhoto')">
              <X class="w-3.5 h-3.5" />
            </button>
        ...
        <div class="lg:col-span-2 space-y-8 animate-in fade-in duration-500">
          ...
          <div v-if="activeTab === 'general'" class="space-y-8">
            <section class="space-y-6">
              <h3 class="text-[11px] font-bold text-txt-muted uppercase tracking-[0.2em] border-l-2 border-txt-main pl-3">{{ $t('profile.basicInfo') }}</h3>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <AppInput v-model="profileForm.firstName" :label="$t('profile.firstName')" :placeholder="$t('profile.firstNamePlaceholder')" />
                <AppInput v-model="profileForm.lastName" :label="$t('profile.lastName')" :placeholder="$t('profile.lastNamePlaceholder')" />
                <AppInput v-model="profileForm.title" :label="$t('profile.jobTitle')" :placeholder="$t('profile.jobTitlePlaceholder')" class="sm:col-span-2" />
                <AppInput v-model="profileForm.email" :label="$t('profile.email')" class="sm:col-span-2" :placeholder="$t('profile.emailPlaceholder')" />
                <AppPhoneInput v-model="profileForm.phone" v-model:countryCode="profileForm.countryCode" :label="$t('profile.phone')" class="sm:col-span-2" />
              </div>
            </section>

            <section class="space-y-6">
              <h3 class="text-[11px] font-bold text-txt-muted uppercase tracking-[0.2em] border-l-2 border-txt-main pl-3">{{ $t('profile.settings') }}</h3>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <AppSelect v-model="profileForm.timezone" :label="$t('profile.timezone')" :options="timezones" />
                <AppSelect v-model="profileForm.language" :label="$t('profile.language')" :options="[{ label: 'Türkçe', value: 'tr' }, { label: 'English', value: 'en' }]" />
              </div>
            </section>
          ...
          <div v-if="activeTab === 'security'" class="space-y-8">
            <section class="space-y-6">
              <h3 class="text-[11px] font-bold text-txt-muted uppercase tracking-[0.2em] border-l-2 border-txt-main pl-3">{{ $t('profile.passwordManagement') }}</h3>
              <div class="space-y-4">
                <AppInput v-model="passwordForm.currentPassword" type="password" :label="$t('profile.currentPassword')" placeholder="••••••••" />
                <AppInput v-model="passwordForm.newPassword" type="password" :label="$t('profile.newPassword')" placeholder="••••••••" />
              </div>
            ...
            <section class="space-y-6">
              <div class="flex items-center justify-between border-l-2 border-txt-main pl-3">
                <h3 class="text-[11px] font-bold text-txt-muted uppercase tracking-[0.2em]">{{ $t('profile.twoFactor') }}</h3>
                <button v-if="!setupData && authStore.hasMfaEnabled" @click="openSetupModal" class="text-[11px] font-bold text-txt-main hover:text-txt-muted transition-colors flex items-center gap-1 cursor-pointer">
                  <Plus class="w-3.5 h-3.5" /> {{ $t('profile.addNewDevice') }}
                </button>
              </div>
              ...
              <div v-if="authStore.totpDevices.length === 0 && !setupData" class="p-6 bg-card border border-line rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
                <div>
                  <h4 class="text-sm font-bold text-txt-main">{{ $t('profile.secureAccount') }}</h4>
                  <p class="text-xs text-txt-muted mt-1 max-w-sm">{{ $t('profile.secureAccountDescription') }}</p>
                </div>
              ...
              <div v-if="setupData" class="p-6 bg-card border border-line rounded-2xl space-y-6 animate-in slide-in-from-top-2">
                <div class="flex justify-between items-start">
                  <h4 class="text-sm font-bold text-txt-main">{{ $t('profile.newDeviceSetup') }}: <span class="text-txt-muted">{{ newDeviceName }}</span></h4>
                ...
                <div class="flex flex-col sm:flex-row gap-8 items-center">
                  ...
                  <div class="space-y-4 flex-1 w-full">
                    <div>
                      <p class="text-[10px] font-bold text-txt-muted uppercase tracking-wider mb-1">{{ $t('profile.setupKey') }}</p>
                      <p class="font-mono text-xs bg-side p-2 rounded select-all border border-line text-txt-main">{{ setupData.secret }}</p>
                    </div>
                    <div class="space-y-2">
                      <p class="text-[10px] font-bold text-txt-muted uppercase tracking-wider">{{ $t('profile.verificationCode') }}</p>
                      <div class="flex gap-2">
                        ...
                        <button @click="completeMfaSetup" :disabled="verifyCode.length !== 6" class="px-4 bg-txt-main text-main rounded-lg text-xs font-bold hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-all">{{ $t('profile.verify') }}</button>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div v-for="device in authStore.totpDevices" :key="device.name" class="flex items-center justify-between p-4 bg-card border border-line rounded-xl group hover:border-txt-main/30 transition-all">
                  ...
                  <div class="flex items-center gap-2">
                    <button v-if="!device.verified" @click="resumeMfaSetup(device.name)" class="px-3 py-1.5 bg-amber-500/10 text-amber-600 hover:bg-amber-500 hover:text-white rounded-lg text-[10px] font-bold transition-all cursor-pointer flex items-center gap-1">
                      <RefreshCw class="w-3 h-3" /> {{ $t('profile.completeSetup') }}
                    </button>
                    <button @click="askRemoveMfa(device.name)" class="p-2 text-txt-muted hover:text-rose-500 hover:bg-rose-500/10 rounded-lg transition-all cursor-pointer" :title="$t('profile.removeDevice')">
    ...
    <AppModal :show="showAvatarModal" :title="$t('profile.uploadProfilePhoto')" size="sm" @close="showAvatarModal = false">
      <div class="space-y-6">
        ...
        <div class="flex items-center gap-3 p-3 rounded-lg bg-side/50 border border-line">
          ...
          <p class="text-[11px] text-txt-muted leading-tight" v-html="$t('profile.supportedFormats') + ' <br>' + $t('profile.maxFileSize')">
          </p>
        </div>
      </div>
      <template #footer>
        <button @click="showAvatarModal = false" class="px-4 py-2 text-xs font-bold text-txt-muted hover:text-txt-main transition-colors">{{ $t('common.cancel') }}</button>
      </template>
    </AppModal>

    <AppModal :show="showDeleteAvatarModal" :title="$t('profile.removeProfilePhoto')" size="sm" @close="showDeleteAvatarModal = false">
      <div class="text-center py-4">
        <div class="w-12 h-12 bg-rose-500/10 text-rose-500 rounded-full flex items-center justify-center mx-auto mb-4">
          <Trash2 class="w-6 h-6" />
        </div>
        <p class="text-sm font-bold text-txt-main">{{ $t('profile.photoWillBeDeleted') }}</p>
        <p class="text-xs text-txt-muted mt-2 max-w-[220px] mx-auto">{{ $t('profile.irreversibleAction') }}</p>
      </div>
      <template #footer>
        <button @click="showDeleteAvatarModal = false" class="px-4 py-2 text-xs font-bold text-txt-muted hover:text-txt-main transition-colors">{{ $t('common.cancel') }}</button>
        <button @click="confirmRemoveAvatar" class="px-6 py-2 bg-rose-500 text-white rounded-lg text-xs font-bold hover:bg-rose-600 transition-colors">{{ $t('profile.yesRemove') }}</button>
      </template>
    </AppModal>

    <AppModal :show="showAddDeviceModal" :title="$t('profile.addDevice')" size="sm" @close="showAddDeviceModal = false">
      <div class="space-y-4">
        <div class="bg-side/50 p-4 rounded-xl text-center">
          <p class="text-sm text-txt-muted">{{ $t('profile.nameYourDevice') }}</p>
        </div>
        <AppInput v-model="newDeviceName" :placeholder="$t('profile.devicePlaceholder')" @keydown.enter="confirmAddDevice" />
      </div>
      <template #footer>
        <button @click="showAddDeviceModal = false" class="px-4 py-2 text-xs font-bold text-txt-muted hover:text-txt-main transition-colors">{{ $t('common.cancel') }}</button>
        <button @click="confirmAddDevice" class="px-6 py-2 bg-txt-main text-main rounded-lg text-xs font-bold hover:opacity-90 transition-opacity">{{ $t('common.continue') }}</button>
      </template>
    </AppModal>

    <AppModal :show="showDeleteDeviceModal" :title="$t('profile.deleteDevice')" size="sm" @close="showDeleteDeviceModal = false">
      <div class="text-center py-4">
        <div class="w-12 h-12 bg-rose-500/10 text-rose-500 rounded-full flex items-center justify-center mx-auto mb-4">
          <Trash2 class="w-6 h-6" />
        </div>
        <p class="text-sm font-bold text-txt-main">{{ $t('profile.deviceWillBeDeleted', { name: deviceToDelete }) }}</p>
        <p class="text-xs text-txt-muted mt-2 max-w-[200px] mx-auto">{{ $t('profile.deviceAccessRemoved') }}</p>
      </div>
      <template #footer>
        <button @click="showDeleteDeviceModal = false" class="px-4 py-2 text-xs font-bold text-txt-muted hover:text-txt-main transition-colors">{{ $t('common.cancel') }}</button>
        <button @click="confirmRemoveDevice" class="px-6 py-2 bg-rose-500 text-white rounded-lg text-xs font-bold hover:bg-rose-600 transition-colors">{{ $t('profile.yesDelete') }}</button>
      </template>
    </AppModal>
...
const tabs = [
  { id: 'general', label: $t('profile.title'), icon: User },
...
const processAvatarFile = (file) => {
  if (!file.type.startsWith('image/')) {
    toast.add($t('toast.invalidImageFile'), 'error');
    return;
  }
  ...
  reader.onload = async (e) => {
    ...
    if (success) {
      ...
      toast.add($t('toast.profilePhotoUpdated'), 'success');
    }
...
const updatePassword = async () => {
  if (!passwordForm.newPassword || !passwordForm.currentPassword) {
    toast.add($t('toast.enterCurrentAndNewPassword'), 'warning');
    return;
  }
  if (passwordForm.newPassword.length < 6) {
    toast.add($t('toast.newPasswordMinLength'), 'warning');
    return;
  }
  ...
  if (success) {
    ...
    toast.add($t('toast.passwordUpdated'), 'success');
  }
