<template>
  <div class="w-full border border-line bg-card rounded-xl overflow-hidden shadow-none transition-all flex flex-col">
    <div class="overflow-x-auto custom-scrollbar relative">
      <table class="w-full text-left border-collapse border-spacing-0 min-w-[700px] lg:min-w-full">
        <thead class="bg-side/40 border-b border-line">
          <tr>
            <th v-for="col in columns" :key="col.key" @click="col.sortable && $emit('sort', col.key)" class="px-4 py-3 text-[10px] font-bold uppercase tracking-wider transition-all select-none group" :class="[
        ...
        <tbody class="divide-y divide-line/40 relative min-h-[200px]">
          <tr v-if="loading">
            <td :colspan="columns.length" class="py-20">
          ...
          <tr v-else v-for="(row, index) in data" :key="index" class="group hover:bg-side/50 transition-colors cursor-pointer" @click="$emit('row-click', row)">
    ...
    <div class="px-4 py-3 border-t border-line bg-side/20 flex flex-col sm:flex-row items-center justify-between gap-4 mt-auto">
      <div class="text-[11px] text-txt-muted font-bold uppercase tracking-tighter order-2 sm:order-1">
        {{ $t('common.total') }} {{ totalItems }} {{ $t('common.records') }}
      </div>
      <div class="flex items-center gap-1 order-1 sm:order-2">
        <button @click="$emit('page-change', currentPage - 1)" :disabled="currentPage === 1 || loading" class="p-1.5 border border-line rounded-md hover:bg-side disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
        ...
        <button @click="$emit('page-change', currentPage + 1)" :disabled="currentPage === totalPages || loading" class="p-1.5 border border-line rounded-md hover:bg-side disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
